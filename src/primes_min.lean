...
begin
 ...
 let m := fact n + 1,
 let p := min_fac m,
 have m_gt_1 : m > 1 := nat.succ_lt_succ (fact_pos n),
...
lemma larger_prime_b : ∀ n : ℕ, ∃ p, (prime p) ∧ (p > n) := 
begin
 intro n,
 let m := fact n + 1,
 let p := min_fac m,
 have p_prime : prime p := min_fac_prime (ne_of_gt (nat.succ_lt_succ (fact_pos n))),
 exact ⟨p,⟨p_prime,
